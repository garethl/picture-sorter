...
impl SpecialHandler for MotionPhoto {
    ...
    fn can_handle(
        ...
        if !options.overwrite && destination_exists {
            return false;
        }
        ...
        if picture_is_motion_photo(picture) {
            ...
            if options.motion_extract && !options.overwrite && motion_video_file.exists() {
                warn!(
